...
export const use_GetUserInfo = () => {
  ...
  const fetch_UserId = async (role: string): Promise<UserInfo> => {
    ...
    // =========================
    let fetchedUserId = sessionStorage.getItem('guest-userId') || ''
    let fetchedHotelKey = sessionStorage.getItem('hotelKey') || ''

    // =========================
    // Fetch from API if missing
    // =========================
    if (!fetchedUserId || !fetchedHotelKey) {
      try {
        ...
        sessionStorage.setItem('hotelKey', fetchedHotelKey)
      } catch (err: any) {
