...
export const use_token_Validation = () => {
  ...
  const handle_Token_Validation = async (status: string,hotelKey:string) => {
    try {
      // Make request to backend validation route with HttpOnly cookies
      const response = await axios.get(`${BaseUrl}guest/check-token/validate?hotelKey=${hotelKey}`, {
      ...
      navigate(`/guest/login/${hotelKey}`, { replace: true })
    }
  }
